...
struct Option
{
    Option() : param(ffm_get_default_param()), nr_folds(1), do_cv(false), on_disk(false), use_default_train(false) {}
    string tr_path, va_path, model_path, g_tr_path, g_va_path, w_tr_path, w_va_path;
    ffm_parameter param;
    ffm_int nr_folds;
    bool do_cv, on_disk, use_default_train;
};
...
Option parse_option(int argc, char **argv)
{
    ...
    Option opt;
    opt.g_tr_path = "../feat/train_without_leak_group_counts.txt";
    opt.g_va_path = "../feat/test_group_counts.txt";
    ...
    for(; i < argc; i++)
    {
        ...
        else if(args[i].compare("-g_tr") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -g_tr");
            i++;
            opt.g_tr_path = args[i];
        }
        else if(args[i].compare("-g_va") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -g_va");
            i++;
            opt.g_va_path = args[i];
        }
        else if(args[i].compare("-w_tr") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -w_tr");
            i++;
            opt.w_tr_path = args[i];
        }
        else if(args[i].compare("-w_va") == 0)
        {
            if(i == argc-1)
                throw invalid_argument("need to specify path after -w_va");
            i++;
            opt.w_va_path = args[i];
        }
        ...
        else if(args[i].compare("--use-default-train") == 0)
        {
            opt.param.use_default_train = true;
        }
...
int train(Option opt)
{
    ...
    ffm_problem *tr = ffm_read_problem(opt.tr_path.c_str(), opt.g_tr_path.c_str(), opt.w_tr_path.c_str());
    if(tr == nullptr)
    {
    ...
    {
        // TODO update hard coding
        va = ffm_read_problem(opt.va_path.c_str(), opt.g_va_path.c_str(), opt.w_va_path.c_str());
        if(va == nullptr)
        {
