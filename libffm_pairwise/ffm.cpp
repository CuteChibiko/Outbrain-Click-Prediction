    ...
    bool do_update=false)
{
    ...
    __m128 XMMweight = _mm_set1_ps(weight);
    __m128 XMMt = _mm_setzero_ps();

    // N*N/2回分のループ(NはNaでない素性のみなので多くはない)
    for(ffm_node *N1 = begin; N1 != end; N1++)
    {
        ffm_int j1 = N1->j;   //フィールド番号
        ffm_int f1 = N1->f;   //素性番号
        ffm_float v1 = N1->v; //値
        ...
        for(ffm_node *N2 = N1+1; N2 != end; N2++)
        {
            ...
            // 対応するフィールドに対するウェイトを取得するためw1はf2をw2はf1を使う
            ...
            if(do_update)
            {
                ...
                for(ffm_int d = 0; d < model.k; d += 4)
                {
                    ...
                    // 勾配
                    // g1 = lambda*w1 + kappa*v1*(v2*r*w2)
                    // g2 = lambda*w2 + kappa*v2*(v1*r*w1)
                    ...
                    // __m128 XMMg1 = _mm_mul_ps(_mm_add_ps( _mm_mul_ps(XMMlambda, XMMw1), _mm_mul_ps(XMMkappav, XMMw2)),XMMweight);
                    // __m128 XMMg2 = _mm_mul_ps(_mm_add_ps( _mm_mul_ps(XMMlambda, XMMw2), _mm_mul_ps(XMMkappav, XMMw1)),XMMweight);

                    //wg1[d] += g1*g1*weight;
                    //wg2[d] += g2*g2*weight;
                    // wg1 += g1*g1
                    // wg2 += g2*g2
                    XMMwg1 = _mm_add_ps(XMMwg1,_mm_mul_ps(XMMweight, _mm_mul_ps(XMMg1, XMMg1)));
                    XMMwg2 = _mm_add_ps(XMMwg2,_mm_mul_ps(XMMweight, _mm_mul_ps(XMMg2, XMMg2)));

                    //w1[d] -= eta*g1*weight/sqrt(wg1[d]);
                    //w2[d] -= eta*g2*weight/sqrt(wg2[d]);
                    // w1 -= eta*g1/√wg1
                    // w2 -= eta*g2/√wg2
                    XMMw1 = _mm_sub_ps(XMMw1, _mm_mul_ps(XMMweight,_mm_mul_ps(XMMeta, _mm_mul_ps(_mm_rsqrt_ps(XMMwg1), XMMg1))));
                    XMMw2 = _mm_sub_ps(XMMw2, _mm_mul_ps(XMMweight,_mm_mul_ps(XMMeta, _mm_mul_ps(_mm_rsqrt_ps(XMMwg2), XMMg2))));
    ...
    for(ffm_node *N1 = begin; N1 != end; N1++)
    {
        ...
        for(ffm_node *N2 = N1+1; N2 != end; N2++)
        {
            ...
            if(do_update)
            {
                ...
                {
                    // 勾配
                    ffm_float g1 = lambda*w1[d] + kappa*v1*(v2*r*w2[d]);
                    ffm_float g2 = lambda*w2[d] + kappa*v2*(v1*r*w1[d]);
...
// サンプル点ごとのバリュー正規化用配列の作成　1/Σvalue^2
...
vector<ffm_float> make_map_j2f(ffm_problem &prob)
{
    vector<ffm_float> map_j2f(prob.n);
    for(ffm_int j = 0; j < prob.n; j++)
    {
        map_j2f[j] = -1;
    }
    for(ffm_int i = 0; i < prob.l; i++)
    {
        for(ffm_long p = prob.P[i]; p < prob.P[i+1]; p++)
        {
            if(prob.X[p].j > prob.n)
                continue;
            map_j2f[prob.X[p].j] = map_j2f[prob.X[p].f];
        }
    }
    return map_j2f;
}
    ...
    ffm_problem *va=nullptr)
{
   ...
   cout << "train ver 13.1" << endl;
    ...
    vector<ffm_float> work_W;
    vector<ffm_float> map_j2f;
    if(auto_stop){
        prev_W.assign(w_size, 0);
        work_W.assign(w_size, 0);
        map_j2f = make_map_j2f(*va);
    }
    //ffm_double best_va_loss = numeric_limits<ffm_double>::max();
    ffm_double best_va_mapk = numeric_limits<ffm_double>::min();
    ...
    for(ffm_int iter = 1; iter <= param.nr_iters; iter++)
    {
        ...
        ffm_double tr_mapk = 0;
        ffm_int tr_group_exec_counts = 0;
        if(param.random)
            random_shuffle(order.begin(), order.end());
        // TODO ランダムソートしないと遅くなる？？？
        if(param.random)
            random_shuffle(group_order.begin(), group_order.end());
#if defined USEOMP
#pragma omp parallel for schedule(static) reduction(+: tr_loss)
#endif
        // グループのループ
        for(ffm_int jj = 0; jj < (ffm_int)group_order.size(); jj++)
        {
          ffm_int j = group_order[jj];

          // デバッグ時の小さいデータ用にスキップ処理
          if(tr->group_cumsum[j] > (ffm_int)order.size())
              continue;

          ffm_int base = tr->group_cumsum[j] - tr->group_counts[j];
          ffm_int counts = (ffm_int)tr->group_counts[j];
          ffm_int i_clicked = -1;
          if(tr->data_weight[base]<=0){
              // グループ内のデータはすべて同じ値を持っているはずなので先頭のデータだけ確認
              continue;
          }
          ffm_float data_weight = tr->data_weight[base];
          for(ffm_int i = base; i < counts + base; i++)
          {
              if(tr->Y[i] == 1){
                  i_clicked = i;
                  break;
          ...
          if(i_clicked == -1)
              cout << "err in:" << j << "\t" << base << "\t" << counts << endl;

          ffm_node *begin_clicked = &tr->X[tr->P[i_clicked]];
          ffm_node *end_clicked = &tr->X[tr->P[i_clicked+1]];
          ffm_float r_clicked = R_tr[i_clicked];
          ffm_float t_clicked = wTx(begin_clicked, end_clicked, r_clicked, *model);
          ffm_double expnyt_clicked = exp(-1*t_clicked);
          tr_loss += log(1+expnyt_clicked);
          ...
          for(ffm_int i = base; i < counts + base; i++)
          {
           if(param.use_default_train){
              //ffm_int i = order[ii];
              ffm_float y = tr->Y[i];
              /* ii番目のトレーニングデータの始まりと終わり */
              ffm_node *begin = &tr->X[tr->P[i]];
              ffm_node *end = &tr->X[tr->P[i+1]];
              ffm_float r = R_tr[i];
              // 内積計算(ii番目のトレーニングデータのNAでない素性のすべての組み合わせの内積)
              ffm_float t = wTx(begin, end, r, *model);
              ffm_float expnyt = exp(-y*t);
              tr_loss += log(1+expnyt);
              ffm_float kappa = -y*expnyt/(1+expnyt);
              //wTx(begin, end, r, *model, kappa, param.eta*data_weight/((ffm_float)counts-1), 1, param.lambda, true); // ウェイト更新
              wTx(begin, end, r, *model, kappa, param.eta*data_weight, 1, param.lambda, true);
           } else {
            if(i == i_clicked)
                continue;
            ...
            ffm_node *begin = &tr->X[tr->P[i]]; /* i番目のトレーニングデータの始まり */
            ffm_node *end = &tr->X[tr->P[i+1]]; /* i番目のトレーニングデータの終わり */
            ffm_float r = R_tr[i];
            ffm_float t = wTx(begin, end, r, *model); // 内積計算(i番目のトレーニングデータのNAでない素性のすべての組み合わせの内積)
            ffm_double expnyt = exp(-y*t);
            tr_loss += log(1+expnyt);
            if(t_clicked - t <= 0){
                ng_count++;
            }
            ffm_float t_comb = t_clicked - t;
            ffm_double expnyt_comb = 1/(1+exp(1*t_comb));
            ffm_float kappa_comb = 1*expnyt_comb/(1+expnyt_comb);
            ffm_float kappa = 1*kappa_comb;
            ffm_float kappa_clicked = -1*kappa_comb;

            wTx(begin, end, r, *model, kappa, param.eta*data_weight/((ffm_float)counts-1), 1, param.lambda, true); // ウェイト更新
            wTx(begin_clicked, end_clicked, r_clicked, *model, kappa_clicked, param.eta*data_weight/((ffm_float)counts-1), 1, param.lambda, true);
           }
          }
          tr_mapk += 1/(float(ng_count)+1);
          tr_group_exec_counts++;
        }

        if(!param.quiet)
        {
            tr_loss /= tr->l;
            tr_mapk /= tr_group_exec_counts;

            cout.width(4);
            cout << iter;
            cout.width(13);
            cout << fixed << setprecision(5) << tr_loss << "\t" << tr_mapk;
            if(va != nullptr && va->l != 0)
            {
                ffm_double va_loss = 0;
                ffm_double va_mapk = 0;
                ffm_int va_group_exec_counts = 0;
//#if defined USEOMP
//#pragma omp parallel for schedule(static) reduction(+:va_loss)
//#endif
                for(ffm_int j = 0; j < (ffm_int)va->group_line; j++)
                {
                  // デバッグ時の小さいデータ用にスキップ処理
                  if(va->group_cumsum[j] > (ffm_int)order.size())
                      continue;
        
                  // グループ内でのループ
                  ffm_int base = va->group_cumsum[j] - va->group_counts[j];
                  ffm_int counts = (ffm_int)va->group_counts[j];
                  ffm_int i_clicked = -1;
                  if(va->data_weight[base]<=0){
                      // グループ内のデータはすべて同じ値を持っているはずなので先頭のデータだけ確認
                      continue;
                  }
                  for(ffm_int i = base; i < counts + base; i++)
                  ...
                  ffm_node *begin_clicked = &va->X[va->P[i_clicked]];
                  ffm_node *end_clicked = &va->X[va->P[i_clicked+1]];
                  ffm_float r_clicked = R_va[i_clicked];
                  ffm_float t_clicked = wTx(begin_clicked, end_clicked, r_clicked, *model);
                  ffm_double expnyt_clicked = exp(-1*t_clicked);
                  va_loss += log(1+expnyt_clicked);
                  ...
                  va_mapk += 1/(float(ng_count)+1);
                  va_group_exec_counts++;
                  //cout << 1/(float(ng_count)+1) << endl;
                }
                va_loss /= va->l;
                va_mapk /= va_group_exec_counts;

                cout.width(13);
                cout << fixed << setprecision(5) << va_loss << "\t" << va_mapk;
                cout << endl;

                if(auto_stop)
                {
                    if(va_mapk < best_va_mapk)
                    {
    ...
    ffm_parameter param)
{
    ...
    // kALIGN = 4の倍数にしている。 k_aligned = 4(param.k=4のとき）のはず
    ffm_int k_aligned = (ffm_int)ceil((ffm_double)param.k/kALIGN)*kALIGN;
    ffm_long w_size = (ffm_long)model->n * model->m * k_aligned * 2;
    vector<ffm_float> prev_W;
    if(auto_stop)
...
ffm_problem* ffm_read_problem(char const *path, char const *group_path, char const *weight_path)
{
    ...
    prob->Y = nullptr;
    prob->group_line = 0;
    prob->group_counts = nullptr;
    prob->group_cumsum = nullptr;
    prob->data_weight = nullptr;
    ...
    if(group_path){
        ...
        ffm_long gline = 0;
        for(; fgets(line, kMaxLineSize, gf) != nullptr;)
            gline++;
        rewind(gf);

        prob->group_line = gline;
        prob->group_counts = new ffm_int[gline];
        prob->group_cumsum = new ffm_int[gline];
        for(gline = 0; fgets(line, kMaxLineSize, gf) != nullptr;gline++){
//cout << line << "\t";
            char *counts = strtok(line, ",");
            char *cumsum = strtok(nullptr, "\n");
            prob->group_counts[gline] = atoi(counts);
            prob->group_cumsum[gline] = atoi(cumsum);
//cout << "[" << atoi(counts) << "]\t" << atoi(cumsum) << endl;
//cout << "[" << counts << "]\t" << cumsum << endl;
        }
    }

    // TODO change hard cording
    // char hard_cording_data_weight[] = "../input/train_weight.txt";
    // char *weight_path = hard_cording_data_weight;
    if(weight_path){
        ...
        ffm_long wline = 0;
        for(; fgets(line, kMaxLineSize, wf) != nullptr;)
            wline++;
        rewind(wf);

//cout << "wline:" << wline << endl;
        prob->data_weight = new ffm_float[wline];
        for(wline = 0; fgets(line, kMaxLineSize, wf) != nullptr;wline++){
            char *weight = strtok(line, "\n");
//cout << line << "\t" << "[" << weight << "]\t" << endl;
            prob->data_weight[wline] = atof(weight);
        }
    }
...
ffm_model* ffm_train_with_validation(ffm_problem *tr, ffm_problem *va, ffm_parameter param)
{
    ...
    vector<ffm_int> group_order(tr->group_line);
    for(ffm_int i = 0; i < tr->group_line; i++)
        group_order[i] = i;

    shared_ptr<ffm_model> model = train(tr, order, group_order, param, va);
    ...
    ffm_parameter param)
{
    ...
    for(ffm_int fold = 0; fold < nr_folds; fold++)
    {
        ...
        // TODO fix dummy args
        shared_ptr<ffm_model> model = train(prob, order1, order1, param);
